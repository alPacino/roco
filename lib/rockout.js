 ...
 * [0...] {String} <tasks list>
 * @example
 * $ roco staging deploy:setup
 * Here 'staging' is a regular task which can be defined in your
 * Roco.coffee and where you can set, for instance, roco.env or
 * roco.hosts instead of defining them through environment
 * variables each time you run the deploy.
 */
exports.perform = function (cmds) {
...
exports.list = function listTasks(how, noDescriptions) {
    Object.keys(about).forEach(function (ns) {
        ...
        about[ns].forEach(function (cmd) {
            fullname = ns ? ns + ':' + cmd.name : cmd.name
            if (noDescriptions || !cmd.description) {
                console.log(' roco', fullname);
            } else {
                console.log(' roco', fullname, '-', cmd.description);
            }
        });
    });
};
...
Roco.prototype.after = function (name, action) {
    afterTask[name] = beforeTask[name] || [];
